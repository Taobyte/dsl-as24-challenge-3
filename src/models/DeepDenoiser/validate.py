...
def visualize_predictions_deep_denoiser(
    model_path: str,
    signal_path: str,
    noise_path: str,
    signal_length: int,
    ...
    for snr in tqdm(snrs, total=len(snrs)):
        assoc = get_signal_noise_assoc(
            signal_path, noise_path, Mode.TEST, size_testset=1000, snr=lambda: snr
        )

        test_dl = torch.utils.data.DataLoader(
            InputSignals(assoc, Mode.TEST, snr), batch_size=n_examples
        )
...
def plot_spectograms(cfg: omegaconf.DictConfig) -> None:
    ...
    with torch.no_grad():
        for snr in tqdm(cfg.snrs, total=len(cfg.snrs)):
            noisy_eq, mask = next(iter(test_dl))
