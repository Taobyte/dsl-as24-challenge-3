...
def baseline_model(seq_length: int, kernel_size: int = 3):
    ...
    x = keras.layers.Conv1D(1, kernel_size=kernel_size, padding="same")(
        inputs
    )  # (B, 6120, 1)
...
def baseline_unet(
    seq_length: int, channel_dims: list[int], channel_base: int, kernel_size: int = 3
):
    dense_dim = 128
    n_heads = 4
    ...
    x = keras.layers.Conv1D(
        channel_base, kernel_size=7, activation=None, padding="same"
    )(inputs)
    ...
    x = TransformerEncoderChollet(channel_dims[0] * channel_base, dense_dim, n_heads)(
        x + encoding
    )
    x = keras.layers.BatchNormalization()(x)
