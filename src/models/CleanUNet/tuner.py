...
def tune_stft_loss_clean_unet_pytorch_optuna(cfg: DictConfig):
    ...
    # dataset
    train_dataset = CleanUNetDatasetCSV(cfg.user.data.csv_path, cfg.model.signal_length, cfg.model.snr_lower, cfg.model.snr_upper, Mode.TRAIN, data_format="channel_first")
    val_dataset = CleanUNetDatasetCSV(cfg.user.data.csv_path, cfg.model.signal_length, cfg.model.snr_lower, cfg.model.snr_upper, Mode.TEST, data_format="channel_first")
    ...
    def objective(trial):
        ...
        val_loss, metrics = train_model(net, optimizer, train_dataset, val_dataset, cfg, sc_lambda=sc_lambda, mag_lambda=mag_lambda, stft_lambda=stft_lambda)
        ...
        logger.info(f"Trial finished with validation loss: {val_loss}")
        logger.info(f"Weighted metrics loss: {metric_loss} | cc_loss: {cc_loss} | pw_loss:{pw_loss}")
