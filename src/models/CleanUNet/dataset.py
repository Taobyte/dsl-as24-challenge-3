...
class CleanUNetDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        self.signal_files = (
            glob.glob(f"{signal_path}/**/*.npz", recursive=True)
            if random
            else sorted(glob.glob(f"{signal_path}/**/*.npz", recursive=True))
        )
        self.noise_files = (
            glob.glob(f"{noise_path}/**/*.npz", recursive=True)
            if random
            else sorted(glob.glob(f"{noise_path}/**/*.npz", recursive=True))
        )
    ...
    def __getitem__(self, idx) -> tuple[ndarray, ndarray]:
        ...
        random_noise_idx = (
            np.random.randint(len(self.noise_files)) if self.random else idx
        )
        ...
        if self.spectogram:
            ...
            eq_stacked = stft(
                eq_stacked, self.fft_size, self.hop_size, self.win_length, self.window
            )  # (3*B, #frames, fft_size // 2 + 1)
            eq_stacked = einops.rearrange(
                eq_stacked, "repeat t c -> repeat c t", repeat=3
            )
