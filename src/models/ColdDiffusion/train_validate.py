...
def create_model_and_optimizer(
    ...
    if cfg.model.continue_from_pretrained:
        model.load_state_dict(
            torch.load(
                cfg.model.pretrained_path, map_location=device, weights_only=True
            )
        )
...
def p_losses(
    ...
    x_noisy = forward_diffusion_sample(
        x_start, x_end, t, sqrt_alphas_cumprod, sqrt_one_minus_alphas_cumprod
    ).to(torch.float32)
    predicted_eq = denoise_model(x_noisy.to(torch.float32), t.to(torch.float32))
    x = x_noisy
    new_x_start = predicted_eq
    predicted_noise = (
        x - get_index_from_list(sqrt_alphas_cumprod, t, x.shape) * predicted_eq
    ) / get_index_from_list(sqrt_one_minus_alphas_cumprod, t, x.shape)
    new_x_end = predicted_noise + predicted_eq
    new_t = torch.randint(0, cfg.model.T, (x.shape[0],), device=device).long()
    new_x_noisy = forward_diffusion_sample(
        new_x_start,
        new_x_end,
        new_t,
        sqrt_alphas_cumprod,
    ...
    new_predicted_eq = denoise_model(
        new_x_noisy.to(torch.float32), new_t.to(torch.float32)
    )
    if loss_type == "l1":
...
def train_one_epoch(
    ...
    sum_train_loss = 0
    for step, (eq_in, noise_in) in tqdm(
        enumerate(zip(tr_dl, tr_dl_noise)), total=len(tr_dl)
    ):
        optimizer.zero_grad()
...
def validate_model(
    ...
    with torch.no_grad():
        for step, (eq_in, noise_in) in tqdm(
            enumerate(zip(val_dl, val_dl_noise)), total=len(val_dl)
        ):
...
def test_model(
    ...
    with torch.no_grad():
        model.eval()
        for eq_in, noise_in in tqdm(
            zip(test_loader, noise_test_loader), total=len(test_loader)
        ):
            # eq_in = eq_in[1][:,args.channel_type,:].unsqueeze(dim=1).to(device)
            # reduce_noise = random.randint(*args.Range_RNF) * 0.01
            # noise_real = (noise_in[1][:,args.channel_type,:].unsqueeze(dim=1) * reduce_noise).to(device)
            ...
            restored_dir = testing.direct_denoising(
                model, signal_noisy.to(device).float(), t
            )
