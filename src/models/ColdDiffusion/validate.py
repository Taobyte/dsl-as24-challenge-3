...
def get_metrics_cold_diffusion(
    ...
    """
    test_dataset = TestColdDiffusionDataset(cfg.user.data.test_data_file)
    ...
    def compute_metrs(eq_batch, prediction, shifts):
        ...
        onset = [
            ...
            for a, b, shift in zip(eq_batch[:, idx, :], prediction[:, idx, :], shifts)
        ]
    ...
    with torch.no_grad():
        for eq_batch, noise_batch, shifts in test_dl:
            ...
            if not cfg.model.sampling:
                ...
                corr, max_amplitude_differences, onset = compute_metrs(
                    eq_batch.numpy(), restored_dir.numpy(), shifts
                )
                ccs.extend(corr)
                ...
                corr, max_amplitude_differences, onset = compute_metrs(
                    eq_batch.numpy(), restored_sample.numpy(), shifts
                )
        ...
        if not cfg.model.sampling:
            ...
            return (
                np.array(ccs_sample),
                np.array(amplitudes_sample),
                np.array(onsets_sample),
            )
...
def visualize_predictions_cold_diffusion(cfg):
    ...
    with torch.no_grad():
        for snr in tqdm(cfg.snrs, total=len(cfg.snrs)):
            ...
            restored_dir = testing.direct_denoising(
                model, noisy.to(device).float(), t
            ).cpu()

            t = cfg.model.T - 1
            restored_sample = testing.sample(
                model, noisy.to(device).float(), t, batch_size=noisy.shape[0]
            ).cpu()
