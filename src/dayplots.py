...
def plot_day(cfg):
    ...
    st_ch_dix = client.get_waveforms(network="CH",station="DIX",location='*',channel='HH*',  
    ...
    cat = client.get_events(starttime=utc -time_window[0]-buffer,endtime=utc +time_window[1] + buffer)  # ETH earthquake catalogue

    st_ch_dix.remove_response(output="VEL", pre_filt=[1/1000, 1/200, 45, 50], water_level=None)  # deconvolve instrument response, go to physical units m/s
    if st_ch_dix[0].stats.sampling_rate > 100:  # downsample to 100sps, pre_filter removes >50Hz
        st_ch_dix.resample(100)  # pre filter

    st_ch_dix.trim(utc -time_window[0],endtime=utc +time_window[1])  # remove buffer/edges

    y_limit = np.percentile(st_ch_dix.select(component="Z")[0].data,99.95)  # use same y scale on raw and bandpass data
    fig = st_ch_dix.select(component="Z").plot(type='dayplot',vertical_scaling_range=y_limit,events=cat)
    ...
    fig = st_ch_dix_bandpass.select(component="Z").plot(type='dayplot',vertical_scaling_range=y_limit,events=cat)
